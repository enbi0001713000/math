...
def test_phase4_error_cases_and_boundaries():
    headers = _auth_header()
    ...
    empty_answers = client.post("/api/v1/units/unit_1/tests/submit", headers=headers, json={"answers": []})
    assert empty_answers.status_code == 409 or empty_answers.status_code == 400

    unknown_unit = client.get("/api/v1/units/does_not_exist", headers=headers)
    assert unknown_unit.status_code == 404

    unknown_question = client.post("/api/v1/questions/does_not_exist/answer", headers=headers, json={"answer": "1"})
    assert unknown_question.status_code == 404


def test_phase4_recommendations_reduce_same_day_duplicates():
    headers = _auth_header()
    first = client.get("/api/v1/recommendations/today?count=2", headers=headers)
    second = client.get("/api/v1/recommendations/today?count=2", headers=headers)
    assert first.status_code == 200
    assert second.status_code == 200

    first_ids = {x["questionId"] for x in first.json()["data"]["items"]}
    second_ids = {x["questionId"] for x in second.json()["data"]["items"]}

    # With enough pool size and duplicate-avoidance logic, overlap should be reduced.
    assert len(first_ids.intersection(second_ids)) < 2


def test_phase4_badge_evaluation_idempotent():
    headers = _auth_header()
    ...
    first = client.post("/api/v1/badges/evaluate", headers=headers)
    second = client.post("/api/v1/badges/evaluate", headers=headers)
    assert first.status_code == 200
    assert second.status_code == 200

    first_awarded = first.json()["data"]["awarded"]
    second_awarded = second.json()["data"]["awarded"]
    assert len(second_awarded) <= len(first_awarded)
