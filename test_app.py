...
def test_unit_start_always_step1():
    res = client.post('/api/v1/units/unit_1/start')
...
def test_submit_test_pass_threshold_80():
    res = client.post('/api/v1/units/unit_1/tests/submit', json={'answers': [{'questionId': 'q_t_1', 'answer': 'A'}]})
    assert res.status_code == 200
    data = res.json()['data']
...
def test_random_recommendations_source_label():
    res = client.get('/api/v1/recommendations/today?count=2')
    assert res.status_code == 200
    data = res.json()['data']
